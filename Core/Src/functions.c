...
float32_t CalcPhaseDiff_deg(void)
{
    ...
    /* Peak amplitude of ch1 in volts, and peak frequency.
     * rfft magnitude of a sinusoid of amplitude A (counts) is A*N/2. */
    {
        float32_t rp = re_bin(s_fft1, kpeak), ip = im_bin(s_fft1, kpeak);
        float32_t mag = sqrtf(rp * rp + ip * ip);
        g_amp_v   = (2.0f * mag / (float32_t)adc_num) * (ADC_VREF_V / ADC_FULLSCALE);
...
void CalcMain(void)
{
    if (main_state == START) {
        ...
        snprintf(line, sizeof(line), "F,%s,%s,%s\r\n", sdeg, samp, sfreq);
        CDC_SendString(line);
    }
