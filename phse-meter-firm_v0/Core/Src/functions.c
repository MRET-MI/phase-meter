...
/* Timed acquisition: stop automatically after g_target_count phase outputs.   */
/* g_target_count == 0 means free-run (RUN). RUNN<n> sets it to n (>0).         */
static volatile uint32_t g_target_count = 0;  /* 0 = free-run, else N samples   */
static volatile uint32_t g_out_count    = 0;  /* F outputs since (re)start      */
...
static uint32_t trig_src   = DEF_TRIG_SRC;    /* 0 = internal TIM6, 1 = external ETR/PE7 */
static uint32_t trig_edge  = DEF_TRIG_EDGE;   /* ETR polarity: 0 = rising, 1 = falling   */
...
/* ============================================================ */
/* TMP126 temperature sensors                                    */
/* ------------------------------------------------------------ */
/* SPI2: CS PB1 / PB2 (2 devices),  SPI3: CS PC13 (1 device).    */
/* Independent driver; shares SPI2/SPI3 with the HMC8073 above.  */
/* Each read saves/restores the SPI Init (16-bit/MSB vs 8-bit/   */
/* LSB), so the two devices coexist on the same bus.             */
/* Set TMP126_ENABLED to 0 to disable the temperature stream.    */
/* ============================================================ */
#ifndef TMP126_ENABLED
#define TMP126_ENABLED 1
#endif
#ifndef TMP126_STREAM_DIV
#define TMP126_STREAM_DIV 10u   /* 100 Hz tick / 10 = 10 Hz temperature stream */
#endif

#if TMP126_ENABLED
/* hspi2 / hspi3 are also declared in the HMC8073 block; a repeated extern
 * declaration is legal and keeps this block usable when HMC8073_ENABLED=0. */
extern SPI_HandleTypeDef hspi2;   /* SPI2: SCK PB13, MOSI PB15, MISO PB14 */
extern SPI_HandleTypeDef hspi3;   /* SPI3: SCK PC10, MOSI PC12, MISO PC11 */

TMP126_HandleTypeDef g_tmp126_1 = { .hspi = &hspi2, .cs_port = GPIOB, .cs_pin = GPIO_PIN_1  };
TMP126_HandleTypeDef g_tmp126_2 = { .hspi = &hspi2, .cs_port = GPIOB, .cs_pin = GPIO_PIN_2  };
TMP126_HandleTypeDef g_tmp126_3 = { .hspi = &hspi3, .cs_port = GPIOC, .cs_pin = GPIO_PIN_13 };

static void TMP126_AppInit(void)
{
    TMP126_InitPins(&g_tmp126_1);
    TMP126_InitPins(&g_tmp126_2);
    TMP126_InitPins(&g_tmp126_3);
}
#endif

/* ============================================================ */
/* MCP41HV51-104 digital potentiometers (TX/RX)                  */
/* ------------------------------------------------------------ */
/* TX: SPI2, /CS = PB3.   RX: SPI3, /CS = PC14.                  */
/* Independent driver; shares SPI2/SPI3 with HMC8073 & TMP126.   */
/* Each access saves/restores the SPI Init (8-bit MSB) so all    */
/* devices coexist on the same bus.                              */
/* Set MCP41HV51_ENABLED to 0 to disable.                        */
/* ============================================================ */
#ifndef MCP41HV51_ENABLED
#define MCP41HV51_ENABLED 1
#endif

#if MCP41HV51_ENABLED
extern SPI_HandleTypeDef hspi2;   /* TX: SCK PB13, MOSI PB15, MISO PB14, /CS PB3  */
extern SPI_HandleTypeDef hspi3;   /* RX: SCK PC10, MOSI PC12, MISO PC11, /CS PC14 */

MCP41HV51_HandleTypeDef g_mcp41hv51_tx = { .hspi = &hspi2, .cs_port = GPIOB, .cs_pin = GPIO_PIN_3  };
MCP41HV51_HandleTypeDef g_mcp41hv51_rx = { .hspi = &hspi3, .cs_port = GPIOC, .cs_pin = GPIO_PIN_14 };

static void MCP41HV51_AppInit(void)
{
    MCP41HV51_InitPins(&g_mcp41hv51_tx);
    MCP41HV51_InitPins(&g_mcp41hv51_rx);
    /* Program the wipers from the saved/default parameters (0..256). */
    MCP41HV51_SetWiper(&g_mcp41hv51_tx, (uint16_t)parm[P_POT_TX].data);
    MCP41HV51_SetWiper(&g_mcp41hv51_rx, (uint16_t)parm[P_POT_RX].data);
}
#endif
...
void Reg_prepare(void)
{
    ...
    reg_set(P_TRIG_SRC,   "trig_src",   UNIT_INT,   DEF_TRIG_SRC,    0.0f);
    reg_set(P_TRIG_EDGE,  "trig_edge",  UNIT_INT,   DEF_TRIG_EDGE,   0.0f);
    reg_set(P_POT_TX,     "pot_tx",     UNIT_INT,   DEF_POT_TX,      0.0f);
    reg_set(P_POT_RX,     "pot_rx",     UNIT_INT,   DEF_POT_RX,      0.0f);
}
...
void Parm_set(void)
{
    ...
    trig_src   = (uint32_t)parm[P_TRIG_SRC].data ? 1u : 0u;
    trig_edge  = (uint32_t)parm[P_TRIG_EDGE].data ? 1u : 0u;
...
{
    /*
     * Make the ADC trigger source self-contained: TIM1 TRGO = Update event,
     * used by both the internal (ARR=239 -> 1 MHz update) and external-clock
     * (ETR, ARR=0 -> update per pulse) modes. Overriding hadc1.Init here means
     * a missing CubeMX change (ExternalTrigConv still = T1_CC1) cannot silently
     * break triggering — including the internal 100 Hz mode.
     */
    ...
    /* The ADC is triggered by TIM1 TRGO = Update event (both internal and
     * external-clock modes). Ensure TRGO = Update regardless of CubeMX. */
    ...
    /* TIM1 and the circular DMA are started in ADC_Start. */
}

/* Select the TIM1 clock source: external = ETR (external clock mode 2, PE7)
 * with 1 pulse -> 1 update -> 1 ADC trigger (ARR=0); internal = APB clock
 * (ARR is set from fs in Parm_set). */
static void Tim1_SetExternalClock(uint8_t external, uint8_t falling)
{
    TIM_ClockConfigTypeDef c = {0};
    if (external) {
        c.ClockSource    = TIM_CLOCKSOURCE_ETRMODE2;
        c.ClockPolarity  = falling ? TIM_CLOCKPOLARITY_FALLING
                                    : TIM_CLOCKPOLARITY_RISING;
        c.ClockPrescaler = TIM_CLOCKPRESCALER_DIV1;
        c.ClockFilter    = 0u;
        HAL_TIM_ConfigClockSource(&htim1, &c);
        __HAL_TIM_SET_AUTORELOAD(&htim1, 0u);   /* 1 pulse = 1 update = 1 trigger */
    } else {
        c.ClockSource = TIM_CLOCKSOURCE_INTERNAL;   /* ARR set from fs in Parm_set */
        HAL_TIM_ConfigClockSource(&htim1, &c);
    }
}

/* Start continuous circular acquisition. In internal mode TIM1 free-runs at fs;
 * in external mode each ETR (PE7) pulse produces one conversion. The DMA loops
 * over adc12_buff forever (started once; never re-armed — robust on H7). */
void ADC_Start(void)
{
    if (main_state == START) return;   /* already running */
    main_state = START;
    g_out_count = 0;                   /* (re)start the timed-acquisition window */
    Tim1_SetExternalClock((uint8_t)(trig_src == 1u), (uint8_t)(trig_edge == 1u));
...
/* 100 Hz tick. In internal-trigger mode this is the compute trigger. dbg_tick
 * keeps advancing in all modes (time base for g_seq and the 10 Hz temp stream). */
void OnTick100Hz(void)
{
    dbg_tick++;
    if (main_state == START && trig_src == 0u) {
        g_process = 1;   /* internal: snapshot + compute on the next main-loop pass */
    }
}

/* Circular DMA wrap = one full adc_num frame. In external-clock mode (ETR) this
 * is the compute trigger (adc_num pulses = one frame); internal mode just counts. */
void OnAdcConvCplt(void)
{
    dbg_conv++;
    if (main_state == START && trig_src == 1u) {
        g_process = 1;   /* external: one frame captured -> compute */
    }
}
...
void CalcMain(void)
{
    if (main_state == START) {
        ...
        /* Timed acquisition: after the requested number of outputs, notify the
         * host and stop (this also halts the temperature stream on the next pass). */
        g_out_count++;
        if (g_target_count && g_out_count >= g_target_count) {
            CDC_SendString("DONE\r\n");
            ADC_Stop();
        }
    }
...
/* ============================================================ */
/* Raw waveform dump (WAVE)                                      */
/* ------------------------------------------------------------ */
/* snapshot_frame() is defined later (main-loop section).       */
/* ============================================================ */
static void snapshot_frame(void);

/* Dump one raw 2ch frame (live snapshot) as ASCII hex:
 *   H,<adc_num>,<fs_hz>\r\n
 *   W,<hex...>\r\n   (32 samples/line, each = ch1(4 hex) + ch2(4 hex))
 *   WEND\r\n
 * Raw 16-bit ADC counts incl. DC. CDC_SendString blocks until the previous
 * packet is sent, so the ~128 chunk lines transfer without loss. */
static void WaveDump(void)
{
    char line[600];
    snprintf(line, sizeof(line), "H,%lu,%lu\r\n",
             (unsigned long)adc_num, (unsigned long)fs_hz);
    CDC_SendString(line);

    const uint32_t PER = 32u;   /* samples per line */
    for (uint32_t i = 0; i < adc_num; i += PER) {
        uint32_t m = (adc_num - i < PER) ? (adc_num - i) : PER;
        char *p = line;
        *p++ = 'W'; *p++ = ',';
        for (uint32_t j = 0; j < m; j++) {
            uint32_t w = s_snapshot[i + j];
            p += sprintf(p, "%04X%04X",
                         (unsigned)(w & 0xFFFFu), (unsigned)((w >> 16) & 0xFFFFu));
        }
        *p++ = '\r'; *p++ = '\n'; *p = '\0';
        CDC_SendString(line);
    }
...
static int ParseSubCmd(const char *tok, char *resp)
{
    ...
    /* RUNN<n>: acquire n phase samples (100 Hz), then auto-stop. Check before RUN. */
    if (strncmp(tok, "RUNN", 4) == 0) {
        int n = atoi(tok + 4);
        if (n <= 0) { snprintf(resp, RESP_SZ, "NG"); return 1; }
        g_target_count = (uint32_t)n;
        ADC_Start();
        snprintf(resp, RESP_SZ, "OK");
        return 1;
    }
    if (strcmp(tok, "RUN") == 0)  { g_target_count = 0; ADC_Start(); snprintf(resp, RESP_SZ, "OK"); return 1; }
    if (strcmp(tok, "STOP") == 0) { g_target_count = 0; ADC_Stop();  snprintf(resp, RESP_SZ, "OK"); return 1; }

    /* WAVE: dump one raw 2ch frame from the live buffer (START only). */
    if (strcmp(tok, "WAVE") == 0) {
        if (main_state != START) { snprintf(resp, RESP_SZ, "NG"); return 1; }
        snapshot_frame();
        WaveDump();
        snprintf(resp, RESP_SZ, "OK");
        return 1;
...
#if TMP126_ENABLED
    /* Temperature sensors on demand: TEMP -> 3 values [C], TID -> device IDs. */
    if (strcmp(tok, "TEMP") == 0) {
        float t1 = 0.0f, t2 = 0.0f, t3 = 0.0f;
        TMP126_ReadTemperature(&g_tmp126_1, &t1);
        TMP126_ReadTemperature(&g_tmp126_2, &t2);
        TMP126_ReadTemperature(&g_tmp126_3, &t3);
        char a[16], b[16], c[16];
        ftoa_dec(t1, a, sizeof(a), 4u);
        ftoa_dec(t2, b, sizeof(b), 4u);
        ftoa_dec(t3, c, sizeof(c), 4u);
        snprintf(resp, RESP_SZ, "%s,%s,%s", a, b, c);
        return 1;
    }
    if (strcmp(tok, "TID") == 0) {
        uint16_t id1 = 0u, id2 = 0u, id3 = 0u;
        TMP126_ReadDeviceId(&g_tmp126_1, &id1);
        TMP126_ReadDeviceId(&g_tmp126_2, &id2);
        TMP126_ReadDeviceId(&g_tmp126_3, &id3);
        snprintf(resp, RESP_SZ, "%04X,%04X,%04X",
    ...
    /* MCP41HV51 pots: POTT<0..256> (transmitter, SPI2) / POTR<0..256> (receiver, SPI3). */
    if (tok[0]=='P' && tok[1]=='O' && tok[2]=='T') {
        int paddr; uint8_t is_tx;
        if      (tok[3]=='T') { paddr = P_POT_TX; is_tx = 1; }
        else if (tok[3]=='R') { paddr = P_POT_RX; is_tx = 0; }
        else { snprintf(resp, RESP_SZ, "NG"); return 1; }

        int v = atoi(tok + 4);
        if (v < 0 || v > POT_WIPER_MAX) { snprintf(resp, RESP_SZ, "NG"); return 1; }
#if MCP41HV51_ENABLED
        MCP41HV51_HandleTypeDef *h = is_tx ? &g_mcp41hv51_tx : &g_mcp41hv51_rx;
        if (MCP41HV51_SetWiper(h, (uint16_t)v) != MCP41HV51_OK) { snprintf(resp, RESP_SZ, "NG"); return 1; }
#else
        (void)is_tx;
#endif
        parm[paddr].data = (int32_t)v;
        snprintf(resp, RESP_SZ, "OK");
        return 1;
    }
...
/* Periodic temperature stream while acquiring (starts on RUN, stops on STOP).
 * Rate-limited off the 100 Hz TIM6 tick counter (dbg_tick): every
 * TMP126_STREAM_DIV ticks read all three sensors and emit one line
 *   T,<tick>,<t1>,<t2>,<t3>\r\n
 * On a read error the sensor's last good value is re-sent. */
static void TempStream(void)
{
#if TMP126_ENABLED
    /* Measure only while ADC acquisition is running (RUN..STOP). */
    if (main_state != START) return;

    static uint32_t last_tick = 0u;
    uint32_t now = dbg_tick;
    if ((uint32_t)(now - last_tick) < TMP126_STREAM_DIV) return;
    last_tick = now;

    float t1 = g_tmp126_1.last_temp_c;
    float t2 = g_tmp126_2.last_temp_c;
    float t3 = g_tmp126_3.last_temp_c;
    TMP126_ReadTemperature(&g_tmp126_1, &t1);
    TMP126_ReadTemperature(&g_tmp126_2, &t2);
    TMP126_ReadTemperature(&g_tmp126_3, &t3);

    char s1[16], s2[16], s3[16], line[64];
    ftoa_dec(t1, s1, sizeof(s1), 4u);
    ftoa_dec(t2, s2, sizeof(s2), 4u);
    ftoa_dec(t3, s3, sizeof(s3), 4u);
    snprintf(line, sizeof(line), "T,%lu,%s,%s,%s\r\n",
             (unsigned long)now, s1, s2, s3);
    CDC_SendString(line);
#endif
}
...
void Main_Processing(void)
{
    ...
    /* 3. periodic temperature stream while running (starts on RUN, stops on STOP) */
    TempStream();
}
