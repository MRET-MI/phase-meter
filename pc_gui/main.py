...
try:
    ...
    from ui_waveform_dialog import Ui_WaveformDialog
except ImportError:
    sys.exit("UI python files missing. Generate them with:\n"
             "    python regen_ui.py")

# 16-bit ADC full-scale, Vref = 3.3 V (raw counts -> volts).
ADC_COUNT_TO_V = 3.3 / 65536.0
...
class WaveformDialog(QDialog, Ui_WaveformDialog):
    """生波形表示サブウィンドウ（Designer: ui/waveform_dialog.ui）。"""
...
class MainWindow(QWidget, Ui_MainWindow):
    ...
    def __init__(self):
        ...
        self._waveform = WaveformDialog(self)   # 生波形はサブウィンドウ
        self._streaming = False                 # デバイスが START(RUN) 中か
        self._wave_last = None                  # (fs, ch1_counts, ch2_counts)
        self._wave_live_timer = QTimer(self)    # Live 時の WAVE 反復
        self._wave_live_timer.timeout.connect(self._capture_waveform)
    ...
    def _init_widgets(self) -> None:
        ...
        # 生波形サブウィンドウの単位
        w = self._waveform
        w.unitCombo.addItem("Volts", "V")
        w.unitCombo.addItem("Counts", "C")
    ...
    def _wire(self) -> None:
        ...
        self.waveformButton.clicked.connect(self._open_waveform)
        self._waveform.captureButton.clicked.connect(self._capture_waveform)
        self._waveform.liveCheck.toggled.connect(self._toggle_wave_live)
        self._waveform.unitCombo.currentIndexChanged.connect(self._redraw_waveform)
        self._waveform.saveWaveButton.clicked.connect(self._save_waveform)
    ...
    def _toggle_connect(self) -> None:
        if self._worker is None:
            ...
            worker.waveform_received.connect(self._on_waveform)
    ...
    def _open_waveform(self) -> None:
        ...
        self._waveform.activateWindow()

    def _capture_waveform(self) -> None:
        """WAVE を送って1フレーム取得。未 RUN なら自動でフリーラン開始してから送る
        （WAVE は START 中のライブバッファを読むため）。"""
        if self._worker is None:
            self._append_log("Not connected.")
            return
        if self._streaming:
            ...
            self._append_log("Waveform: 自動 RUN（待機中だったため）")
            self._send("RUN")            # フリーラン開始（データクリアはしない）
            self._streaming = True
            QTimer.singleShot(250, lambda: self._send("WAVE"))

    def _toggle_wave_live(self, on: bool) -> None:
        if on:
            hz = max(0.5, self._waveform.liveRateSpin.value())
            self._wave_live_timer.start(int(1000.0 / hz))
        else:
            self._wave_live_timer.stop()

    def _on_waveform(self, fs: int, ch1, ch2) -> None:
        self._wave_last = (fs, ch1, ch2)
        self._redraw_waveform()

    def _redraw_waveform(self) -> None:
        if self._wave_last is None:
            return
        fs, ch1, ch2 = self._wave_last
        n = min(len(ch1), len(ch2))
        dt = 1.0 / fs if fs > 0 else 1.0
        x = [i * dt for i in range(n)]
        volts = self._waveform.unitCombo.currentData() != "C"
        if volts:
            y1 = [c * ADC_COUNT_TO_V for c in ch1[:n]]
            y2 = [c * ADC_COUNT_TO_V for c in ch2[:n]]
            self._waveform.wavePlot.configure_units("Voltage", "V")
        else:
            y1 = list(ch1[:n]); y2 = list(ch2[:n])
            self._waveform.wavePlot.configure_units("ADC", "counts")
        self._waveform.wavePlot.set_frame(x, y1, y2)
        self._waveform.waveInfoLabel.setText(f"{n} pts @ {fs:,} Hz")

    def _save_waveform(self) -> None:
        if self._wave_last is None:
            self._append_log("No waveform captured yet.")
            return
        fs, ch1, ch2 = self._wave_last
        n = min(len(ch1), len(ch2))
        default = datetime.now().strftime("wave_%Y%m%d_%H%M%S.csv")
        path, _ = QFileDialog.getSaveFileName(self, "Save waveform", default, "CSV (*.csv)")
        if not path:
            return
        dt = 1.0 / fs if fs > 0 else 1.0
        try:
            with open(path, "w", newline="") as f:
                wr = csv.writer(f)
                wr.writerow([f"# fs_hz={fs}"])
                wr.writerow(["time_s", "ch1_count", "ch2_count", "ch1_v", "ch2_v"])
                for i in range(n):
                    wr.writerow([f"{i * dt:.9f}", ch1[i], ch2[i],
                                 f"{ch1[i] * ADC_COUNT_TO_V:.6f}",
                                 f"{ch2[i] * ADC_COUNT_TO_V:.6f}"])
            self._append_log(f"Saved waveform ({n} pts) to {path}")
        except OSError as exc:
            self._append_log(f"[error] waveform save failed: {exc}")
    ...
    def _on_sample(self, seq: int, deg: float, amp: float, freq: float) -> None:
        self._streaming = True   # F サンプル受信＝デバイスは START 中
    ...
    def _on_line(self, line: str) -> None:
        ...
        if line == "DONE":
            self._streaming = False   # 自動停止でデバイスは WAIT へ
