...
class WaveformPlot(pg.PlotWidget):
    """Whole-trace 2ch waveform plot (not rolling). Promotable in Qt Designer:
        class name : WaveformPlot
        header     : plot_widget
    The controller replaces the entire trace each capture via set_frame().
    """
    ...
    def __init__(self, parent=None):
        ...
        self._legend = self.addLegend()
        self._ch1 = self.plot(pen=pg.mkPen("#1565c0", width=1), name="ch1")
        self._ch2 = self.plot(pen=pg.mkPen("#c62828", width=1), name="ch2")

    def configure_units(self, ylabel: str = "Voltage", yunits: str = "V") -> None:
        self.setLabel("left", ylabel, units=yunits)

    def set_frame(self, x, ch1, ch2) -> None:
        """Replace both traces. x/ch1/ch2 are equal-length sequences."""
        x = np.asarray(x, dtype=float)
        self._ch1.setData(x, np.asarray(ch1, dtype=float))
        self._ch2.setData(x, np.asarray(ch2, dtype=float))
